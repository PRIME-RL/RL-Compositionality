...
def random_expr(depth=3):
    ...
    if random.random() < 0.2:  # Binary branch.
        ...
        if r < 0.02:
            ...
            param_custom = [x for x in param_custom if x in param_custom]
            if random.random() < 0.5:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--local_dir', default="data/synthetic_data/synthetic_string_manipulation_composition_1level_incomplete_train_set")
    ...
    for set_idx, custom_functions in enumerate([train_set]):
        forward_data = []
        backward_data = []
        ...
        if set_idx == 0:
            depths = [1]
            total_samples_per_depth = 500000
        elif set_idx == 1:
            depths = [1, 2, 3, 4, 5, 6]
            total_samples_per_depth = 512
        for depth in depths:
