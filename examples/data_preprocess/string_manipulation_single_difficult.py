...
def _get_base_expr():
    """
    Helper function to generate a depth-0 expression.
    Returns either the variable 'x' or a random constant literal string.
    """
    # if random.random() < 0.5:
    #     return "x"
    # else:
    literal = "".join(random.choices(string.ascii_lowercase, k=random.randint(1, 6)))
    return f"'{literal}'"


def random_expr(depth=1):
    """
    Generates a random string expression of depth 1 in terms of 'x'
    by uniformly selecting from a list of all possible operations.

    Args:
        depth (int): The depth of the expression, assumed to be 1.
    """
    # The 'depth' parameter is kept for signature consistency, but the logic
    # assumes it will be 1, making sub-expressions have depth 0.
    if depth != 1:
        return _get_base_expr()

    # A single, consolidated list of all operations for uniform selection.
    # all_ops = [
    ...
    # weights = {
    #     "backchain_palindrome": 418,
    #     "backchain_add_digit": 661,
    #     "deterministic_shuffle": 3117,
    #     "recursive_interlace": 3583,
    #     "shift_chars": 3642,
    #     "alternate_case": 6624,
    #     "interlace_str": 7090,
    #     "compress_repeats": 7439,
    #     "while_rotate": 7952,
    #     "recursive_reverse": 8922,
    #     "vowel_to_number": 14710,
    #     "reverse_words": 14721,
    #     "rotate_str": 15412,
    #     "duplicate_every_char": 16852,
    #     "mirror_str": 18089,
    #     "verify_even_length": 18216,
    # }
    ...
    weights = {
        ...
        'repeat_str': 65913,
        'add_suffix': 87453,
        'insert_separator': 39089,
        'sort_chars': 22770,
        'verify_even_length': 18223,
        'add_prefix': 47441,
        'remove_vowels': 22327,
        'rotate_str': 15536,
        'recursive_reverse': 8980,
        'while_rotate': 7983,
        'upper': 79444,
        'duplicate_every_char': 16871,
        'shift_chars': 3654,
        'compress_repeats': 7446,
        'vowel_to_number': 14729,
        'deterministic_shuffle': 3128,
        'loop_filter_nonalpha': 20868,
        'fancy_brackets': 20227,
        'reverse_words': 14734,
        'backchain_add_digit': 666,
        'alternate_case': 6634,
        'backchain_palindrome': 418,
        'recursive_interlace': 3594
    }
    ...
    op = np.random.choice(weight_keys, p=[weights[k] for k in weight_keys])

    # --- Construct the expression based on the chosen operation ---

    # 1. Binary operations
    if op in ["interlace_str", "recursive_interlace"]:
        left = _get_base_expr()
        right = _get_base_expr()
        # if op == "+":
        #     return f"({left} + {right})"
        # else:
    ...
    elif op in ["upper", "lower", "capitalize", "swapcase"]:
        sub_expr = _get_base_expr()
        return f"({sub_expr}).{op}()"

    # 3. Custom functions with one argument
    elif op in [
            "deterministic_shuffle",
            "remove_vowels",
            "sort_chars",
    ...
    # 4. Custom functions with a second, randomly generated parameter
    else:
        sub_expr = _get_base_expr()
        if op == "repeat_str":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--local_dir",
        default="data/synthetic_data/synthetic_string_manipulation_single_difficult",
    )
    ...
    for depth in depths:
        for i, data in enumerate([forward_data, backward_data]):
            if i == 1 and depth == 1:
                continue
            count = 0
    ...
    dataset["train"].to_parquet(f"{args.local_dir}/forward_train.parquet")
    dataset["test"].to_parquet(f"{args.local_dir}/forward_test.parquet")
    dataset["test"].select(range(100)).to_parquet(f"{args.local_dir}/forward_debug.parquet")
    print(
