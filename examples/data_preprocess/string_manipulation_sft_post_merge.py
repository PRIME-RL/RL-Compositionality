...
dataset1 = load_dataset("parquet", data_files="data/string_manipulation_sft/train.parquet")
dataset2 = load_dataset("parquet", data_files="data/string_manipulation_sft_difficult/train.parquet")
...
op_acc = {}
for data in chain(dataset1['train'], dataset2['train']):
    for op in all_ops:
...
for op, data_list in op_acc.items():
    ...
    final_dataset += data_list[:4096]  # Take 4096 samples for each operation

final_dataset = Dataset.from_list(final_dataset)
print(final_dataset)
final_dataset.to_parquet("data/string_manipulation_sft_merged/train_balanced.parquet")