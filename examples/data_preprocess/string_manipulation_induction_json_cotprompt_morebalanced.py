...
# --------------------------------------------------
SYNTHESIS_PROMPT = """You are given {num_examples} input-output pairs of a Python function.

Here are the inputs and outputs:
{examples}

The underlying python function is composed of the following function(s):
```python
{functions}
```

Determine the correct one-line function composition (using the provided functions) that transforms each input to its corresponding output. The input variable will be x in your solution. Analyze the input-output pairs to deduce how the functions are composed. Verify your solution works for all given examples. Provide your answer as a one-line function call with json format at the end of your answer: {{"code": <your output>}}, where <your output> should be the one-line function call, e.g., func1(func2(x))"""
...
def random_expr(depth=3):
    ...
    if random.random() < 0.1:  # Binary branch.
...
if __name__ == "__main__":
    ...
    parser.add_argument('--local_dir', default="data/synthetic_data/synthetic_string_manipulation_induction_with_testcase_json_level12_500k_cot_balanced")
    args = parser.parse_args()

    depths = [1, 2]
    total_samples_per_depth = 250512
    synthesis_data = []
    generated_exprs = set()
    random.seed(42)
    ...
    for depth in depths:
        ...
        while count < total_samples_per_depth:
            ...
            sample = {
                "data_source": f"codeio-induction-json-depth{depth}",
